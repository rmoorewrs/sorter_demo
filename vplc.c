...
/*
 * 160121-initial code written, Rich Moore, richard.moore@windriver.com,
...
int plc_state_write (plc_t *plc, plc_state_t *new_state)
{
    ...
    /* copy in new state variable table and restore status and input count */
...
{
    /*
     * Now we actually talk to the remote modbus slave/server device, write
     * the outputs, read inputs and update the state variable table (SVT).
     *
     * First: write outputs and read them back via the modbus_write_bits()
     * and modbus_read_bits() functions, these read/write to the outputs
     * only ("coils" in modbus termin0logy).
     *
     * Second: read inputs via the modbus_read_input_bits() function. This
     * relates only  to the digital inputs.
     *
     * Note: if analog I/O is added that is a different set of registers and
     * modbus functions
     */
    uint8_t       ndi, ndo, status;
    int           i, rc;
    ...
    /*
     * modbus functions use the buffer passed in for return values,
     * so make a copy to avoid corrupting the SVT
     */
    ...
    if  (plc->mode == MODE_ACTIVE) {
        ...
        /*
         * First: read back the value written to the outputs and if read is
         * successful, copy back to SVT.
         * FIXME: should probably compare the values written to the values
         * read back
         */
        rc = modbus_read_bits (plc->mb_slave, 0, ndo, obuf);
        if (rc != ndo) {
            printf ("plc_state_write: ERROR reading outputs from modbus (%d)",
                    rc);
            printf ("Address = %d, value = %.2x\n", 0,obuf[0]);
            return (-1);
        }
    ...
    /*
     * set status flag to clean because we just synched
     * with remote successfully
     */
...
plc_t *plc_init_modbus (char* net_addr,
                        int instance,
                        int net_port,
                        int modbus_addr,
                        int nDI,
                        ...
                        int debug_flag)
{
    ...
    plc->state = malloc (sizeof (plc_state_t));
    if (!plc->state) {
        fprintf (stderr, "plc_init: error allocating memory");
        free (plc);
        return (NULL);
    ...
    /*
     * init state variable table, set all values to false,
    ...
    /* connect to remote modbus tcp slave (server).
     * we're modbus master(client) */
    plc->mb_slave = modbus_new_tcp (net_addr, net_port);

    /* set address of remote modbus slave id we want to talk to */
    modbus_set_slave(plc->mb_slave, modbus_addr);

    /* print detailed modbus debugging info or not */
    modbus_set_debug(plc->mb_slave,debug_flag);

    /* also set the local flag for quick reference */
    plc->verbose=debug_flag;
    ...
    if (modbus_connect (plc->mb_slave) == -1) {
        fprintf (stderr,
                 "plc_init: Connection to modbus slave %d failed: %s\n",
    ...
    strncpy (plc->net_addr, net_addr, 17); /* ip address string should be
                                            * < 16 bytes: aaa.bbb.ccc.ddd */
...
static void *hb_client (void *param) 
{
    ...
    while (hb_client_run) {
        ...
        status = SSAPI_mesg_AsyncSend (plc->conn, hbmsg, sizeof(hbmsg_t));
        if (status < 0) {
