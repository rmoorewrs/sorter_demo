...
#define LOOP_PERIOD_USEC	40000	// 20Hz is a period of 50ms or 50000usec
	...
	while (1) {
		...
		case STATE_WAITING_FOR_BOX_AT_CHUTE :
			if (svt->DI[DI_RETROREF_NOT_02]==FALSE){
				...
				svt->DO[DO_PUSHER_01]=svt->DO[DO_PUSHER_02]=svt->DO[DO_PUSHER_03]=FALSE;
			}
		...
		case STATE_BOX_SORTED :
			...
			// make sure the pushers are back, stop blades are up, emitter off, etc
			svt->DO[DO_STOP_BLADE_01]=svt->DO[DO_STOP_BLADE_02]=TRUE;
			svt->DO[DO_PUSHER_01]=svt->DO[DO_PUSHER_02]=svt->DO[DO_PUSHER_03]=FALSE;
